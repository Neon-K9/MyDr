                                        ...
                                        <div class="card-block pb-0">
                                            <form class="link-form" action='{{ route('users.update', $user->id) }}' method='POST' enctype="multipart/form-data">
                                                ...
                                                @if ($user->type === 'patient')
                                                    <input class='form-control mt-3' type='text' name='social' placeholder="Mobile number" value='{{ $user->patient->social_number ?? '' }}'>
                                                    <div class="form-group mt-2">
                                                        <label for="blood-type">Blood type</label>
                                                        <select class="form-control" id="blood-type" name="blood">
                                                            <option value="{{ $user->patient->blood_type ?? '' }}">{{ $user->patient->blood_type ?? 'Select' }}</option>
                                                ...
                                                @endif
                                                @if ($user->type === 'doctor')
                                                    <input class='form-control mt-3' type='text' name='specialty' placeholder="specialty" value='{{ $user->doctor->specialty ?? '' }}'>
                                                @endif
                                                <br>
